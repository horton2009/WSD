...
#         Peter Ljungl枚f <peter.ljunglof@heatherleaf.se> (additions)
...
class LidstoneProbDist(ProbDistI):
    ...
    *gamma* to the count for each bin, and taking the maximum
    ...
    """
    SUM_TO_ONE = False
    def __init__(self, freqdist, gamma, bins=None):			# 加 gamma 平滑，分子 + gamma; 分母 + bins×gamma
        """
        ...
        if bins is None: bins = freqdist.B()				# key 的个数
        self._bins = bins

        self._divisor = self._N + bins * gamma				# 分母 + bins×gamma
    ...
    *(c+1)/(N+B)*.  This is equivalant to adding one to the count for
    ...
    def __init__(self, freqdist, bins=None):
        ...
        :param bins: The number of sample values that can be generated
            ...
            ``bins`` is not specified, it defaults to ``freqdist.B()``.
        """
        LidstoneProbDist.__init__(self, freqdist, 1, bins)			# 拉普拉斯平滑为加一平滑
    ...
    *B* bins as *(c+0.5)/(N+B/2)*.  This is equivalant to adding 0.5
    ...
    def __init__(self, freqdist, bins=None):
        ...
        :param bins: The number of sample values that can be generated
            ...
            ``bins`` is not specified, it defaults to ``freqdist.B()``.
        """
        LidstoneProbDist.__init__(self, freqdist,0.5, bins)			# Gamma 默认值为 0.5
