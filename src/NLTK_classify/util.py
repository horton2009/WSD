...
def names_demo_features(name):				# 特征提取方法，字符串变列表 
    features = {}
    features['alwayson'] = True					# 设置一个常有的特征！！！
    features['startswith'] = name[0].lower()
    features['endswith'] = name[-1].lower()
    for letter in 'abcdefghijklmnopqrstuvwxyz':		# 共有 26×2+3 个特征
        features['count(%s)' % letter] = name.lower().count(letter)			# features：为 dict 结构
...
def binary_names_demo_features(name):
    ...
    for letter in 'abcdefghijklmnopqrstuvwxyz':		# 共有 26×4+3 个特征
...
def names_demo(trainer, features=names_demo_features):
    ...
    try:
        ...
        ll = [pdist.logprob(gold)		# gold：正确答案，正确答案的对数概率
              for ((name, gold), pdist) in zip(test, pdists)]			# zip（list1,list2）取两个列表对应位置元素（以短列表为基础）组建二元组列表
        print 'Avg. log likelihood: %6.4f' % (sum(ll)/len(test))		# 所以测试样本的平均对数似然相似度（似然相似度区对数，积变和）
        print
